import { User, Role, Assessment, QuestionType, AssessmentResult, LessonPlan, LessonSubmission, Notification, AssessmentSubmission, CustomLesson, CustomLessonSubmission } from '../types';
...
export const mockUsers: User[] = [
  ...
  { userId: 'teacher-3', name: 'Mr. Khalid', email: 'mr.khalid@school.ae', role: Role.TEACHER, schoolId: 'uae-school-1', classId: 'class-1', subject: 'Maths' },
...
export const mockAssessments: Assessment[] = [
  {
    ...
    title: 'Grade 5 - Science - Chapter 3',
    subject: 'Science',
    grade: '5',
    teacherId: 'teacher-1',
    classId: 'class-1',
...
export const mockAssessmentResults: AssessmentResult[] = [
  ...
  // Math (assessment-2) — so Mr. Khalid sees pre-results too
...
export const mockLessonPlans: LessonPlan[] = [
  {
    ...
    assessmentId: 'assessment-1',
    masteryTasks: {
      students: ['student-1', 'student-5'],
      tasks: 'Research and create a presentation on the advanced stages of photosynthesis and its importance for ecosystems. Link: nationalgeographic.com'
    },
    developingTasks: {
      students: ['student-2', 'student-3'],
      tasks: 'Complete the worksheet on identifying different planets in our solar system. Review chapter 3.2. Link: phet.colorado.edu'
    },
    needsSupportTasks: {
      students: ['student-4'],
      tasks: 'Work one-on-one with the teacher to review the basic concept of photosynthesis using flashcards and a simplified diagram. Link: brainpopjr.com'
    },
    iepConsidered: true,
  }
];
...
export const mockLessonSubmissions: LessonSubmission[] = [
  { submissionId: 'sub-1', assessmentId: 'assessment-1', studentId: 'student-2', submissionContent: 'I have completed the worksheet on planets. Mars is red because of iron oxide on its surface.', fileName: 'worksheet_noora.pdf', status: 'graded', grade: 'approved', feedback: 'Great work on the worksheet!', timestamp: Date.now() - 2 * 24 * 60 * 60 * 1000 },
  { submissionId: 'sub-2', assessmentId: 'assessment-1', studentId: 'student-4', submissionContent: 'I drew a picture of the sun and a plant.', status: 'submitted', timestamp: Date.now() - 1 * 24 * 60 * 60 * 1000 }
];
