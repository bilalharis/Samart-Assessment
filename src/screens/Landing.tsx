...
const ROLE_META: Record<RoleKey, { label: string; Icon: React.ComponentType<any>; gradient: string }> = {
  ...
  [Role.STUDENT]:   { label: 'Student',   Icon: UserIcon, gradient: 'from-pink-accent to-pink-accent' },
};
          ...
          return (
            <div key={role} className="group relative">
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 flex flex-col items-center">
                <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-white shadow`}>
      ...
      <Modal isOpen={!!openRole} onClose={() => setOpenRole(null)} title="Select a user to login">
        {openRole && (
          <div className="space-y-2">
            {mockUsers.filter(u => u.role === openRole).map(u => (
              <button
                key={u.userId}
              ...
              >
                ...
                <span className="text-xs px-2 py-1 rounded bg-gray-100">{ROLE_META[openRole].label}</span>
              </button>
            ))}
